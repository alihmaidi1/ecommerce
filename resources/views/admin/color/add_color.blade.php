...
<form  class="col-sm-12 col-md-6  " method="POST" action="{{ route("save_color") }}" enctype="multipart/form-data">
    @csrf
    ...
    <input  type="color" value="{{ old("color") }}" class="form-control @error('color')is-invalid @enderror" name="color" placeholder="{{ __("messages.Enter color") }}"/>
    @error('color')
    <span class="invalid-feedback " role="alert">
        <strong>{{ $message }}</strong>
    </span>
@enderror
    ...
    <br/>
    <div class="m-auto d-flex justify-content-center"><button class="btn btn-primary w-75">{{ __("messages.Create color") }}</button></div>
</form>
</div>
  </div>
