<div class="tab-pane fade show " id="product_additional">
    ...
    <div class="input-product m-auto">
        ...
        <select style="background:rgb(28, 41, 42);color:#fff;font-weight:800"   multiple name="color_id[]" class="form-control @error('color_id')is-invalid @enderror"> 
            ...
            <option class="p-2 "  value="{{ $color->id }}"
                @foreach(\App\Models\color_product::where("product_id",$product->id)->get() as $value)
    ...
    <div class="input-product m-auto">
        ...
        <input value="{{ $product->tax }}"  value="{{ $product->tax }}" class="form-control @error('tax')is-invalid @enderror" name="tax" placeholder="{{ __("messages.Enter tax") }}"/>
        @error('tax')
        <span class="invalid-feedback " role="alert">
            <strong>{{ $message }}</strong>
        </span>
    ...
    <div class="input-product m-auto">
        ...
        <input value="{{ $product->shipping }}"  value="{{ $product->shipping }}" class="form-control @error('shipping')is-invalid @enderror" name="shipping" placeholder="{{ __("messages.Enter shipping") }}"/>
        @error('shipping')
        <span class="invalid-feedback " role="alert">
            <strong>{{ $message }}</strong>
        </span>
